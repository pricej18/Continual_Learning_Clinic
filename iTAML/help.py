...
def createSaliencyMap(model, sal_loader, pred, ses):
    ##### Create Saliency Maps
    dataiter = iter(sal_loader)
    salImgs, salLbls = next(dataiter)
    salImgs, salLbls = salImgs.cuda(), salLbls.cuda()
    predicted = pred.squeeze()
    
    
    model.setSaliency(True)
    saliency = Saliency(model)
    print(salLbls)
    
    fig, ax = plt.subplots(1,4,figsize=(10,4))
    for ind in range(0,2):
        input = salImgs[ind+1].unsqueeze(0)
        input.requires_grad = True

        grads = saliency.attribute(input, target=salLbls[ind+1].item(), abs=False)
        grads = grads.squeeze().cpu().detach().numpy()

        print('Truth:', classes[salLbls[ind+1]])
        print('Predicted:', classes[predicted[ind+1]])
       
        original_image = salImgs[ind+1].cpu().detach().squeeze().numpy()
        
        for i in range(2):
            if ind==0: plt.subplot(1,4,i+1)
            else: plt.subplot(1,4,i+3)
            ...
            if i % 2 == 0:
                ...
                plt.title('Saliency Map', fontsize=10)

        plt.figtext(1, 0.5, "Sess " + str(ses), fontsize = 20)
        fig.tight_layout()
        plt.show()     
    
    fig.savefig(f"SaliencyMaps/Sess{ses}SalMap.png")
    fig.show()  
    model.setSaliency(False)
