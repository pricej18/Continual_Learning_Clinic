...
def create_diff_heatmap(algorithm, dataset, ses, compare_dict, vmin = 0, vmax = 350):
    ...
    for i in range(size):
        for j in range(size):
            plt.annotate('{:.0f}'.format(data[size-1-i][j]), xy=(j+0.5, i+0.5), ha='center', va='center', color='black')
    ...
    plt.title(f'Difference Metrics for Session {ses}', horizontalalignment='center')
    plt.figtext(0.5, 0.035, f'Standard Deviation: {np.std(data):.2f}', ha='center')
    plt.tight_layout()
    #plt.show(block=False)
    plt.savefig(f"SaliencyMaps/{algorithm}/{dataset}/Sess{ses}Metrics.png")
    #plt.show()
